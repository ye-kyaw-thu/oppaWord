...
def analyze_errors(ref_lines: List[str], hyp_lines: List[str], top_k: int = 10) -> Dict:
    """
    Analyze and categorize segmentation errors
    Returns: dictionary of error statistics
    """
    error_stats = {
        'over_segmentation': Counter(),  # System splits what should be together
        'under_segmentation': Counter(),  # System joins what should be separate
        'incorrect_boundaries': Counter(),  # Both split and join errors
        'total_errors': 0
    }
    ...
    for ref, hyp in zip(ref_lines, hyp_lines):
        ...
        while ref_idx < len(ref_words) and hyp_idx < len(hyp_words):
            ...
            if ref_word == hyp_word:
                ...
                # Case 1: Over-segmentation (system split where it shouldn't)
                if len(hyp_word) < len(ref_word):
                    # Find how many hyp words make up this ref word
                    combined_hyp = hyp_word
                    end_hyp_idx = hyp_idx + 1
                    while (end_hyp_idx < len(hyp_words) and 
                           len(combined_hyp) + len(hyp_words[end_hyp_idx]) < len(ref_word)):
                        combined_hyp += " " + hyp_words[end_hyp_idx]
                        end_hyp_idx += 1
                    
                    if combined_hyp == ref_word:
                        error_key = f"REF: '{ref_word}' → HYP: '{'|'.join(hyp_words[hyp_idx:end_hyp_idx])}'"
                        error_stats['over_segmentation'][error_key] += 1
                        hyp_idx = end_hyp_idx
                        ref_idx += 1
                        continue

                # Case 2: Under-segmentation (system joined what should be separate)
                if len(hyp_word) > len(ref_word):
                    # Find how many ref words make up this hyp word
                    combined_ref = ref_word
                    end_ref_idx = ref_idx + 1
                    while (end_ref_idx < len(ref_words) and 
                           len(combined_ref) + len(ref_words[end_ref_idx]) < len(hyp_word)):
                        combined_ref += " " + ref_words[end_ref_idx]
                        end_ref_idx += 1
                    
                    if combined_ref == hyp_word:
                        error_key = f"REF: '{'|'.join(ref_words[ref_idx:end_ref_idx])}' → HYP: '{hyp_word}'"
                        error_stats['under_segmentation'][error_key] += 1
                        ref_idx = end_ref_idx
                        hyp_idx += 1
                        continue

                # Case 3: Complex errors (both split and join differences)
                error_key = f"REF: '{ref_word}' → HYP: '{hyp_word}'"
                error_stats['incorrect_boundaries'][error_key] += 1
                ref_idx += 1
                hyp_idx += 1

    # Get top-k errors for each category
    for category in ['over_segmentation', 'under_segmentation', 'incorrect_boundaries']:
        error_stats[category] = dict(error_stats[category].most_common(top_k))
