...
for EXP in "${EXPERIMENTS[@]}"; do
  ...
  time python oppa_word.py \
    ...
    --visualize-dag \
    --dag-output-dir "$DAG_VIS_DIR" \
    ${OPTS[$EXP]}

  # Evaluate: simple accuracy
  echo "--- Evaluating tag precision..."
  python2.7 ./tools/evaluate.py "$SEG_FILE" "$REF" | tee "$RESULT_FILE"

  # Evaluate: Top-K token errors
  echo "--- Evaluating top-k frequent token errors..."
  python ./tools/eval_segmentation.py -H "$SEG_FILE" -r "$REF" --top-k 50 > "$TOPK_FILE"

  echo "Saved: $SEG_FILE"
  echo "Saved: $RESULT_FILE"
  echo "Saved: $TOPK_FILE"
  echo "DAG visualizations in: $DAG_VIS_DIR"
  echo
done
